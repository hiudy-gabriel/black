...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `eai @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\nMais um gado para a colheita❤️`
				...
				teks = `Adeusinho @${num.split('@')[0]} ja foi tarde 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '👉aguarde to fazendo👈',
				success: '✔️deu certinho✔️',
				error: {
					stick: '⚠️erro sem figurinha hoje⚠️',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
			...
			const ownerNumber = ["5533999983921@s.whatsapp.net"] 
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`Opoha o ${tipe} nao virou figurinha`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('erro tenta denovo so que mais tarde.')
							})
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADORES:* HIUDY:WA.ME/5533999983921  gabriel:wa.me/558281075291  nickspcx:wa.me/5522997544803'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*nois tem canal n*'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
